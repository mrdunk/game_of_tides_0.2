...
struct Site * VoronoiDiagramGenerator::leftreg(struct Halfedge *he)
{
	if(he -> ELedge == (struct Edge *)NULL) 
		return(bottomsite);
	return( he -> ELpm == VO_LE ? 
		he -> ELedge -> reg[VO_LE] : he -> ELedge -> reg[VO_RE]);
}
...
struct Site * VoronoiDiagramGenerator::rightreg(struct Halfedge *he)
{
	...
	return( he -> ELpm == VO_LE ? he -> ELedge -> reg[VO_RE] : he -> ELedge -> reg[VO_LE]);
}
...
struct Site * VoronoiDiagramGenerator::intersect(struct Halfedge *el1, struct Halfedge *el2, struct PointVDG *p)
{
	...
	if ((right_of_site && el -> ELpm == VO_LE) || (!right_of_site && el -> ELpm == VO_RE)) 
		return ((struct Site *) NULL);
...
int VoronoiDiagramGenerator::right_of(struct Halfedge *el,struct PointVDG *p)
{
	...
	if(right_of_site && el -> ELpm == VO_LE) return(1);
	if(!right_of_site && el -> ELpm == VO_RE) return (0);
	...
	return (el->ELpm==VO_LE ? above : !above);
}
...
void VoronoiDiagramGenerator::endpoint(struct Edge *e,int lr,struct Site * s)
{
	e -> ep[lr] = s;
	ref(s);
	if(e -> ep[VO_RE-lr]== (struct Site *) NULL) 
		return;
...
void VoronoiDiagramGenerator::out_ep(struct Edge *e)
{
	...
	//{	
		//printf("e %d", e->edgenbr);
        	//printf(" %d ", e->ep[VO_LE] != (Site *)NULL ? e->ep[VO_LE]->sitenbr : -1) ;
        	//printf("%d\n", e->ep[VO_RE] != (Site *)NULL ? e->ep[VO_RE]->sitenbr : -1) ;
...
bool VoronoiDiagramGenerator::voronoi(bool genVertexInfo)
{
	...
	while(1)
	{
		...
		//	out_site(newsite);						//output the site
			...
			bisector = HEcreate(e, VO_LE);					//create a new HalfEdge, setting its ELpm field to 0			
			...
			lbnd = bisector;						
			bisector = HEcreate(e, VO_RE);					//create a new HalfEdge, setting its ELpm field to 1
		...
		{	
			...
			pm = VO_LE;								//set the pm variable to zero
			...
			endpoint(e, VO_RE-pm, v);					//set one endpoint to the new edge to be the vector point 'v'.
